...
**설정**: 초기 3 물리 노드, 가상 노드 150개/물리노드 (Ketama 표준 160과 유사).
트래픽은 **Zipfian(s=1.0) 분포** 100,000 요청 × 1000 고유 키 — 상위 10개 키가 전체의 39%를 차지하는 인기키 편중 환경.
측정 시점은 초기 상태에서 `ADD(+1)` 또는 `REMOVE(-1)` 이후.
